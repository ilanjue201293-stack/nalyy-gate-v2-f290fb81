  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        if (!key || key.scriptId !== params.scriptId || key.revoked) {
          return renderAccessDenied("Invalid or revoked key.");
        }
        ...
        if (key.expiresAt && key.expiresAt < new Date()) {
          return renderAccessDenied("This key is expired.");
        }
        if (!key.redeemedByDiscordId) {
          return renderAccessDenied("Redeem this key in Discord before using the loader.");
        }
        ...
        if (whitelist.expiresAt && whitelist.expiresAt < new Date()) {
          return renderAccessDenied("Whitelist access expired.");
        }
        ...
        return new Response(key.script.obfuscate ? obfuscateLua(key.script.scriptContent) : key.script.scriptContent, {
          headers: {
            "content-type": "text/plain; charset=utf-8",
            "cache-control": "no-store",
          },
